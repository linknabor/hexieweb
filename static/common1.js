   ...
   getoriApp:function() {
    ...
    alipayAuthorize() {
        ...
        if(o === undefined) { //没有code获取code
            ...
            let oid = getParam(window.location.search,"user_id");
            if(oid == null) {// 没有拿到openid
                let url = MasterConfig.C("baseUrl") +"/authorizeAlipay/"+o;
                $.ajax({
                    type:'post',
                    url:url,
                    dedaType:'json',
    ...
    weixinAuthorize() {
        ...
        if( o === undefined) { //没有code授权触发
            ...
            if(oid == null) { //没拿到openid
                let url = MasterConfig.C("baseUrl") + "/authorize/"+ o + "/" + MasterConfig.C("appId");
                $.ajax({
                    type:'post',
                    url:url,
                    dataType:'json',
